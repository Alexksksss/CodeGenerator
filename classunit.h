...
public:
    explicit ClassUnit(const std::string& name) : m_name( name ) {
        m_fields.resize(ACCESS_MODIFIERS_SHARP.size());// Инициализация вектора полей для каждого модификатора доступа
    }
    ...
    void add( const std::shared_ptr< Unit >& unit, Flags flags ) {
        ...
        if( flags < ACCESS_MODIFIERS_SHARP.size() ) // Проверка корректности флага доступа
